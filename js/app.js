  ...
  function init() {
    if (document.getElementById('chkBox')) {
      window.Quiz.choice('chkBox', 'chkNote', [
        { k: '1', q: '通信中は回線を占有するので、時間あたりに送れるデータ量が安定する。', ch: ['回線交換方式', 'パケット交換方式'], a: 0,
          why: '1本の道を貸し切るので、速さが変わりにくいのが回線交換の長所です。' },
        { k: '2', q: '回線の数より多くの人が同時に通信できる。', ch: ['回線交換方式', 'パケット交換方式'], a: 1,
          why: 'すき間に他の人のパケットを流せるので、同じ設備でより多くの人が使えます。' },
        { k: '3', q: 'データを送っていない間も回線を使い続けるので、むだが多い。', ch: ['回線交換方式', 'パケット交換方式'], a: 0,
          why: '話していない間も電話がつながったままなのと同じで、空いている時間がむだになります。' },
        { k: '4', q: '1本の回線に、別々のあて先のデータを混ぜて流せる。', ch: ['回線交換方式', 'パケット交換方式'], a: 1,
          why: 'パケットごとにあて先が書いてあるので、混ざっていても正しく届きます。' },
        { k: '5', q: '混んでくると、届くまでの時間が変わりやすい。', ch: ['回線交換方式', 'パケット交換方式'], a: 1,
          why: '順番待ちが起きるためです。動画通話が混雑時に乱れるのはこれが理由です。' }
      ], '仕分けの根拠は下の表で確かめられます。<strong>「安定するが むだが多い」のが回線交換、「効率はよいが 時間が変わりやすい」のがパケット交換</strong>です。');
    }
